...
const ONLY_TYPE_BUTTON_ALERT = "//button[contains(text(),'Change Only Type To Success')]";
...
describe('kintoneUIComponent - Alert', function () {
    ...
    it('[Alert-4-5] should create alert with no option', function () {
        let classNameOfSelectedItem = $(NO_OPTION_ALERT).getAttribute('class');
        let alertIsEnabled = isVisibled(NO_OPTION_ALERT,true)
        let disableAlert = browser.isEnabled(NO_OPTION_ALERT);
        expect(classNameOfSelectedItem).to.equal('kuc-alert bg-danger');
    ...
    it('[Alert-6] should create alert with full option', function () {
        let visibleAlert = browser.isVisible(FULL_OPTION_ALERT);
        let disableAlert = browser.isEnabled(FULL_OPTION_ALERT);
        let typeAlert = $(FULL_OPTION_ALERT).getAttribute('class');
        let textAlert = browser.getText(FULL_OPTION_ALERT);
        expect(visibleAlert).to.equal(true);
        expect(typeAlert).to.equal('kuc-alert bg-danger');
        expect(disableAlert).to.equal(true);
        expect(textAlert).to.equal('full option Alert');
    });

    it('[Alert-7] should create alert with only options.text', function () {
        let isContentVisibled = isVisibled(ONLY_TEXT_ALERT,true);
        expect(isContentVisibled).to.equal(true);
        expect($(ONLY_TEXT_ALERT).getText()).to.equal('Alert with only text');
    });

    it('[Alert-10] should create alert with only options.type', function () {
        let classNameOfSelectedItem = $(ONLY_TYPE_ALERT).getAttribute('class');
        expect(classNameOfSelectedItem).to.equal('kuc-alert bg-danger');
        $(ONLY_TYPE_BUTTON_ALERT).click();
        classNameOfSelectedItem = $(ONLY_TYPE_ALERT).getAttribute('class');
        expect(classNameOfSelectedItem).to.equal('kuc-alert bg-success');
    });

    // it('[Alert-12] should create alert with only options.isDisabled', function () {
    //     let alertIsEnabled = $(ONLY_DISABLED_ALERT).getAttribute('class');
    //     expect(alertIsEnabled).to.equal('true');
    // });

    it('[Alert-14] should create alert with only options.Visible is true', function () {
        let alertIsVisibled = isVisibled(ONLY_VISIBLE_ALERT, true);
        expect(alertIsVisibled).to.equal(true);
    });

    it('[Alert-15] should create alert with only options.Visible is false', function () {
        let alertIsInvisibled = isVisibled(ONLY_INVISIBLE_ALERT, false);
        expect(alertIsInvisibled).to.equal(false);
    });
